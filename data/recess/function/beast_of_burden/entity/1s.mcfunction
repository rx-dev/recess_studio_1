execute \
    ...
    if entity @e[type=zombie,tag=recess.beast_of_burden]

data modify storage recess:main input set value {}
execute store result storage recess:main input.damage float 0.1 run scoreboard players get @s recess.nearby_beasts
