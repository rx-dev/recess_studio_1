...
execute store result storage infection:temp input.roll double 0.1 run random value -25..25
execute store result storage infection:temp input.pitch double 0.1 run random value -25..25
execute \
    if predicate {condition: "random_chance", chance: 0.6} \
    rotated as @s \
    anchored eyes \
    positioned ^ ^ ^0.5 \
    summon marker \
    run function recess:infection/item/flamethrower/init_flame with storage infection:temp input
...
execute if predicate {condition: "random_chance", chance: 0.3} run playsound minecraft:block.fire.ambient player @a ~ ~ ~ .05 2
execute if predicate {condition: "random_chance", chance: 0.15} run playsound minecraft:block.fire.extinguish player @a ~ ~ ~ .3 1.5
execute if predicate {condition: "random_chance", chance: 0.35} run playsound minecraft:block.fire.extinguish player @a ~ ~ ~ .2 .1
